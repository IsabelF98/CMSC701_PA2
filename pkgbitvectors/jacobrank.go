...
func RankSupport(bv *bitvector.BitVector) *RankStruct {
	...
	*/
	...
	var rank_data RankStruct
	rank_data.BV = bv
	rank_data.CumulativeRank = bitvector.NewBitVector(cumulative_rank, len(cumulative_rank))
	rank_data.RelCumulRank = bitvector.NewBitVector(relative_cumu_rank, len(relative_cumu_rank))

	return &rank_data
}

func (rank_data *RankStruct) Rank(idx int) int {
	/*
	...
	*/
	...
	rank1 := rank_data.CumulativeRank.Bytes()[idx1]       // rank of chunk
	rank2 := rank_data.RelCumulRank.Bytes()[idx_aux+idx2] // rank of sub-chunk
